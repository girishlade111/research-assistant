...
[Project Overview](#-project-overview) • [Key Features](#-key-features) • [Development Stack](#-development-stack) • [Configuration](#-configuration) • [Installation](#-installation--usage) • [Project Stats](#-project-stats--metrics) • [Contributing](#-contribution-guidelines)
...
**ResAgent** is a production-grade, multi-agent AI research system designed for depth, accuracy, and scale. Unlike conventional chatbots that rely on a single model pass, ResAgent orchestrates a **fleet of specialized AI agents** that operate concurrently across a four-phase pipeline:

1. **Query Intelligence** — Deconstructs and expands user intent into structured research vectors.
2. **Data Aggregation** — Performs concurrent web searches and ingests user-uploaded documents.
3. **Parallel Synthesis** — Runs Analysis, Summary, Fact-Check, and Coding agents simultaneously.
4. **Report Assembly** — Compiles all outputs into a cohesive, citation-rich research document.

The result is a **4,000–6,000 word structured report** with executive summaries, citation graphs, expert insights, and optional code generation — delivered in real time via Server-Sent Events (SSE).

> **💡 Unique Selling Point:** ResAgent features **Dynamic Model Routing** with automatic fallback to high-capacity context models (up to **131,072 tokens**) when primary endpoints hit rate limits, ensuring uninterrupted massive report generation under heavy load.
...
- **Multi-Modal Document Intake** — Seamlessly parse user-uploaded documents to enrich local context:
  - **PDFs** via `pdfjs-dist`
...
| :--- | :--- | :--- | :--- |
| **Query Intelligence** | Expands raw queries into 8–12 structured research vectors | `moonshotai/kimi-k2-thinking` | `openai/gpt-oss-120b:free` |
...
- **Intent-Based Routing** — Automatically classifies queries as *Simple* (direct chat) or *Research* (full multi-agent pipeline).
...
- **Responsive Design** — Fully adaptive layout with mobile viewport optimization and collapsible sidebar.
...
### Internal Token & Timeout Configuration
The engine uses hard-coded thresholds optimized for massive context management:
...
3. **Optionally upload files** (PDF, DOCX, CSV, images) to enrich context.
4. **Toggle individual agents** on/off via the search controls for customized pipelines.
5. **Watch real-time progress** via the Agent Status Panel as the system streams the final report.
6. **Export** the result as Markdown, PDF, or Plain Text.
